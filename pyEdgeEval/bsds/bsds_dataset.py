...
class BSDSHEDAugDataset(object):
    ...
    def _data_path(self, data_type, scale, rot, flip, name, ext):
        ...
        if data_type not in {"data", "gt"}:
            raise ValueError(
                "data_type should be 'data' or 'gt', not {}".format(data_type)
            )
        ...
        if rot not in self.AUG_ROTS:
            raise ValueError(
                "rot should be one of {}, not {}".format(self.AUG_ROTS, rot)
            )
        if flip not in self.AUG_FLIPS:
            raise ValueError(
                "flip should be one of {}, not {}".format(self.AUG_FLIPS, flip)
            )
        return os.path.join(
            self.root_path,
            fold,
            "aug_{}{}".format(data_type, scale),
            "{}_{}".format(rot, flip),
